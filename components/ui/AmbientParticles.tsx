  ...
  const animate = useCallback((time: number) => {
    ...
    stars.forEach((star, i) => {
      ...
      if (mouseDist < mouseInfluence) {
        const opacity = (1 - mouseDist / mouseInfluence) * 0.4
        ctx.beginPath()
      ...
      for (let j = i + 1; j < stars.length; j++) {
        ...
        if (dist < connectionDistance) {
          ...
          if (mouseToMid < mouseInfluence) {
            opacity += (1 - mouseToMid / mouseInfluence) * 0.2
          }

          ctx.beginPath()
          ctx.moveTo(star.x, star.y)
          ctx.lineTo(other.x, other.y)
          ctx.strokeStyle = `${lineColor}${Math.min(opacity, 0.4)})`
          ctx.lineWidth = 1
          ctx.stroke()
        }
      }
    ...
    // Draw stars with subtle pulse and glow
    stars.forEach((star) => {
      // Very subtle pulse - barely noticeable
      const pulse = Math.sin(time * star.pulseSpeed * 0.3 + star.pulsePhase) * 0.08 + 0.92
      const size = star.size * pulse

      // Glow boost when mouse is near
      const mouseDistX = mouse.x - star.x
      const mouseDistY = mouse.y - star.y
      const mouseDist = Math.sqrt(mouseDistX * mouseDistX + mouseDistY * mouseDistY)
      const glowBoost = mouseDist < mouseInfluence ? (1 - mouseDist / mouseInfluence) * 0.4 : 0

      const baseOpacity = star.opacity * 0.6 // Dimmer base
      const opacity = Math.min(baseOpacity + glowBoost, 0.9)
